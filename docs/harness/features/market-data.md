...
10. `GET /api/market/history/{ticker}?period=...` routes by ticker type:
   ...
   - Exception: `^NDX` (Nasdaq 100) uses Stooq (`^ndx`) as its **primary** source because FMP free/stable does not serve the index. Tickers in `STOOQ_PRIMARY_SYMBOLS` use Stooq even when `ENABLE_STOOQ_FALLBACK=false`, as long as `STOOQ_API_KEY` is set. FMP is not called for `^NDX`, so its daily budget/cooldown is not spent on a symbol FMP cannot return. `^GSPC` keeps the FMP-first + opt-in Stooq fallback behavior. This `^NDX` primary path is scoped to the index; the `KRW=X` change path is separately key-gated via `STOOQ_FX_SYMBOLS` (see the USD/KRW item below), and the STOCK_US current-price Stooq fallback still depends on `ENABLE_STOOQ_FALLBACK`. See `docs/harness/nasdaq-index-stooq-primary-implementation-2026-06-09.md`.
...
- Live provider allowlist: `MARKET_LIVE_TICKERS=DGS10,XAU,BTC-USD,NVDA,005930.KS,^GSPC,^NDX,KRW=X,^KS11` by default. This is still a demo-scoped allowlist, not full production realtime coverage. Non-allowlisted tickers use local `demo_mock` values for prices, news, latest context, and history so demo deployments do not spend free API quota across the full asset universe. Use `*` to disable the allowlist only when broad live provider coverage is intentionally approved.
