...
- Most provider requests are serialized per provider (`Semaphore(1)`). When many assets share one provider (e.g. `stooq` for US history, `naver_news`), the queue can be slow; assets that exceed the per-asset timeout are skipped for that run. Raising concurrency for those is deferred because it risks free-tier rate limits / IP blocks on Stooq and Naver. See `docs/harness/market-data-warmup-provider-throttle-timeout-plan-2026-06-03.md`.
