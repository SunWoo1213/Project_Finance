...
즐겨찾기한 자산의 새 저장 리포트 알림을 Gmail/Telegram으로 보낼 때 리포트 본문을 직접 전달하지 않고, 자산 상세정보 페이지 링크와 현재 가격을 함께 안내하는 형식으로 전환한다.
...
- `docs/harness/feature-index.md`
  - 이 계획서와 향후 구현 기록을 Favorite asset notifications 및 Asset detail change records에 연결한다.
