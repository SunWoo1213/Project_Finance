...
- `REPORT_SCHEDULER_STARTUP_DELAY_SECONDS`: backend 기동 후 리포트 생성 잡의 **첫 발화까지 지연 시간(초)**. 기본값 `60`. `generate_daily_reports`(interval) 잡의 `next_run_time`으로 쓰여, interval 최초 발화가 +1주기 후로 밀리지 않고 기동 직후에 1회 실행되도록 한다(이후는 `REPORT_SCHEDULER_INTERVAL_HOURS` 주기). sleep/재시작형 런타임에서 인스턴스가 일찍 죽어도 첫 발화를 놓치지 않도록 짧게 둔다. 0/음수 입력 시 0으로 보정된다.
